...
namespace PRTileArea {
	...
	public class TileTool : EditorTool {
		...
		private bool DrawButton(Texture2D icon) {
			return DrawButton(icon, icon.width);
		}
		...
		private void DrawTileMenu() {
			...
			if (tiles == null || tiles.Length <= 0) tiles = GetAllTiles();
				...
				for (int i = 0; i < tiles.Length; i++) {
					...
					if (DrawButton(tiles[i].GetThumbnail(), 32)) paintingTile = tiles[i];
					GUILayout.EndArea();
				}
			}
		}
